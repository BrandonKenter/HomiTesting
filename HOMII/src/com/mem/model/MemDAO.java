...
public class MemDAO implements MemDAO_interface {
	...
	public void insert(MemVO memVO) {
		...
		try {
			...
			pstmt.setBytes(4, memVO.getMb_pic());
			pstmt.setString(5, memVO.getMb_phone());
			pstmt.setString(6, memVO.getMb_address());
			pstmt.setString(7, memVO.getMembership());
	...
	public void front_update(MemVO memVO) {
		...
		try {
			...
			pstmt.setString(1, memVO.getMb_name());
			pstmt.setString(2, memVO.getMb_phone());
			pstmt.setString(3, memVO.getMb_address());
			pstmt.setInt(4, memVO.getMember_no());
	...
	public void back_update(MemVO memVO) {
		...
		try {
			...
			pstmt.setString(5, memVO.getMb_phone());
			pstmt.setString(6, memVO.getMb_address());
			pstmt.setDate(7, memVO.getCrt_dt());
			pstmt.setString(8, memVO.getStatus());
			pstmt.setInt(9, memVO.getMember_no());
	...
	public MemVO email_check(String mb_email) {
		...
		try {
			...
			throw new RuntimeException("Create account fail : The email is used" + le.getMessage());
			// Clean up JDBC resources
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
	...
	/*
	 * @Override public Set<RelationshipVO> getRelationshipsByMemberno(Integer
	 * member_no) { Set<RelationshipVO> set = new LinkedHashSet<RelationshipVO>();
	 ...
	 * try {
	 * 
	 * con = ds.getConnection(); pstmt =
	 * con.prepareStatement(GET_Relationships_ByMemberno_STMT); pstmt.setInt(1,
	 * member_no); rs = pstmt.executeQuery();
	 * 
	 * while (rs.next()) {
	 * 
	 ...
	 * 
	 * // Handle any SQL errors } catch (SQLException se) { throw new
	 ...
	 * e.printStackTrace(System.err); } } } return set; }
	 */
	...
	/*
	 * @Override public List<MemVO> getAll(Map<String, String[]> map) { List<MemVO>
	 * list = new ArrayList<MemVO>(); MemVO memVO = null;
	 * 
	 * Connection con = null; PreparedStatement pstmt = null; ResultSet rs = null;
	 * 
	 * try { con = ds.getConnection(); String finalSQL = "select * from `member` " +
	 * jdbcUtil_CompositeQuery_Member.get_WhereCondition(map) +
	 * "order by member_no"; pstmt = con.prepareStatement(finalSQL);
	 * System.out.println("¡´¡´finalSQL(by DAO) = "+finalSQL); rs =
	 * pstmt.executeQuery();
	 * 
	 * while (rs.next()) { memVO = new MemVO();
	 * memVO.setMember_no(rs.getInt("member_no"));
	 * memVO.setMb_name(rs.getString("mb_name"));
	 * memVO.setMb_email(rs.getString("mb_email"));
	 * memVO.setMb_pwd(rs.getString("mb_pwd")); memVO.setMb_bd(rs.getDate("mb_bd"));
	 * memVO.setMb_pic(rs.getBytes("mb_pic"));
	 * memVO.setMb_phone(rs.getString("mb_phone"));
	 ...
	 * } catch (SQLException se) { se.printStackTrace(System.err); } } if (con !=
	 * null) { try { con.close(); } catch (Exception e) {
	 * e.printStackTrace(System.err); } } } return list; }
	 */
}
